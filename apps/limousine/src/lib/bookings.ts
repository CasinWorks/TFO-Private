...
export function bookingToIcs(b: BookingInquiry): string {
  ...
  const summary = `Iceland Limousine · ${b.vehicleName} · ${b.guestName}`;
  const description = [
    `Ref: ${b.id}`,
  ...
  return [
    'BEGIN:VCALENDAR',
    'VERSION:2.0',
    'PRODID:-//Iceland Limousine//Booking//EN',
    'CALSCALE:GREGORIAN',
    'METHOD:PUBLISH',
    'BEGIN:VEVENT',
    `UID:${b.id}@icelandlimousine.com`,
...
export function downloadIcs(b: BookingInquiry) {
  ...
  a.href = url;
  a.download = `iceland-limo-${b.id}.ics`;
  a.click();
...
export function googleCalendarUrl(b: BookingInquiry): string {
  ...
  const params = new URLSearchParams({
    action: 'TEMPLATE',
    text: `Iceland Limousine · ${b.vehicleName} · ${b.guestName}`,
    dates: `${start}/${end}`,
    details: `Ref ${b.id}\n${b.guestName}\n${b.guestPhone}\n${b.guestEmail}\nPickup: ${b.pickup}\nDrop-off: ${b.dropoff}`,
    location: b.pickup,
  });
...
export function outlookCalendarUrl(b: BookingInquiry): string {
  ...
  const params = new URLSearchParams({
    ...
    rru: 'addevent',
    subject: `Iceland Limousine · ${b.vehicleName} · ${b.guestName}`,
