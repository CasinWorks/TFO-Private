...
const linkClass = {
  nav: 'hover:text-white transition-colors inline-flex items-center gap-1.5',
      ...
      <span
        className={
          variant === 'nav'
            ? 'text-[#C5A880]'
            : variant === 'mobile'
              ? 'block w-full text-left text-[#C5A880] text-xl font-serif-luxury tracking-normal normal-case'
              : 'text-[#C5A880]'
        }
      >
        TFO Jets
      </span>
      <a href={SIBLING.limousineBook} className={cls}>
        Iceland Limousine
      </a>
    </>
  );
